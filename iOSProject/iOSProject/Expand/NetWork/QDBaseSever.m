                      ...
                      failure:(void (^)(NSURLSessionDataTask* task, NSError* error))failure
{
    if (@available(iOS 10.0, *)) {
        ...
        NSInteger netState = [app currentNetworkState];
        if (netState == NotReachable) {
            NSDictionary* userInfo = [NSDictionary dictionaryWithObject:NETWorkErrorString forKey:NSLocalizedDescriptionKey];
            NSError* error = [NSError errorWithDomain:NETWorkErrorString code:NETWorkErrorCode userInfo:userInfo];
            failure(nil, error);
            return nil;
        }
    } else {
